    ...
    for domain in domains:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                console.print(f"  ✗ {domain}: [yellow]Access denied (check permissions)[/yellow]")
...
def linkedin_to_yaml(data):
    ...
    for record in data:
        ...
        if domain == 'PROFILE':
            ...
            loc = (
                p.get('Geo Location') or 
                p.get('locationName') or 
                p.get('geoCountryName') or 
                p.get('Address', '')
            )
            if loc:
        ...
        # --- EXPERIENCE ---
            ...
            for job in elements:
                ...
                start = parse_date(
                    job.get('Started On') or job.get('startDate'), 
                    is_start_date=True
                )
                end = parse_date(
                    job.get('Finished On') or job.get('endDate'), 
                    is_start_date=False
                )
        ...
        # --- EDUCATION ---
            ...
            for edu in elements:
                ...
                start = parse_date(
                    edu.get('Start Date') or edu.get('startDate'), 
                    is_start_date=True
                )
                end = parse_date(
                    edu.get('End Date') or edu.get('endDate'), 
                    is_start_date=False
                )
        ...
        # --- CERTIFICATIONS ---
            ...
            for cert in elements:
                ...
                date = parse_date(
                    cert.get('Started On') or cert.get('startDate'), 
                    is_start_date=True
                )
        ...
        # --- PUBLICATIONS ---
            ...
            for pub in elements:
                entry = {
                    ...
                    "date": parse_date(
                        pub.get('Published On') or pub.get('date'), 
                        is_start_date=True
                    ),
                }
        ...
        # --- HONORS ---
            ...
            for honor in elements:
                entry = {
                    ...
                    "date": parse_date(
                        honor.get('Issued On') or honor.get('date'), 
                        is_start_date=True
                    ),
                }
...
def get_custom_domains() -> List[str]:
    ...
    while True:
        ...
        # Validate input
        invalid = [d for d in requested if d not in optional_domains and d not in core_domains]
        if invalid:
            console.print(f"\n[red]❌ Invalid domains: {', '.join(invalid)}[/red]")
            console.print(f"[yellow]Please choose from: {', '.join(optional_domains)}[/yellow]\n")
        else:
...
def linkedin_command(
    token: str = typer.Option(..., prompt="LinkedIn Access Token", help="OAuth 2.0 Access Token with Member Data Portability scope"),
    custom: bool = typer.Option(False, "--custom", help="Interactively select which LinkedIn sections to include"),
    output: str = typer.Option("LinkedIn_CV.yaml", "--output", "-o", help="Output YAML filename")
):
    """
    ...
    if not cv_inner.get("email"):
        new_email = Prompt.ask("  [yellow]?[/yellow] Email is missing. Enter it (or press Enter to skip)", default="")
        cv_inner["email"] = new_email if new_email else None
            
    if not cv_inner.get("phone"):
        new_phone = Prompt.ask("  [yellow]?[/yellow] Phone is missing. Enter it (or press Enter to skip)", default="")
        cv_inner["phone"] = new_phone if new_phone else None
    
    if not cv_inner.get("location"):
         new_loc = Prompt.ask("  [yellow]?[/yellow] Location is missing. Enter it (or press Enter to skip)", default="")
