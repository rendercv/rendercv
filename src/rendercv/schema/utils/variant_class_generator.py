...
__all__ = ["_create_variant_class"]


def _validate_defaults_against_base(
...
def _generate_class_name(variant_name: str, class_name_suffix: str) -> str:
...
def _deep_merge_nested_object[T: pydantic.BaseModel](
    base_nested_obj: T,
    ...
    for key, value in updates.items():
        ...
        if isinstance(value, dict):
            ...
            if isinstance(current_value, pydantic.BaseModel):
                merged_updates[key] = _deep_merge_nested_object(current_value, value)
            else:
...
def _create_nested_field_spec(
    ...
    if base_nested_obj is not None:
        ...
        modified_nested = _deep_merge_nested_object(base_nested_obj, default_value)
...
def _create_variant_class[T: pydantic.BaseModel](
    variant_name: str,
    defaults: dict[str, Any],
    base_class: type[T],
    ...
    _validate_defaults_against_base(defaults, base_class, variant_name)
    ...
    for field_name, default_value in defaults.items():
        ...
        if field_name == discriminator_field:
            field_specs[field_name] = _create_discriminator_field_spec(
                default_value, base_field_info
            )
        # Handle nested objects (dict values)
        elif isinstance(default_value, dict):
            field_specs[field_name] = _create_nested_field_spec(
                default_value, base_field_info
            )
        # Handle simple fields
        else:
            field_specs[field_name] = _create_simple_field_spec(
                default_value, base_field_info
            )

    # Step 3: Generate the class name
    class_name = _generate_class_name(variant_name, class_name_suffix)
